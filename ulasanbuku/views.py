...
def create_ulasan(request, bukuid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ulasan.bukuid = buku
            ulasan.save()
            messages.success(request, 'Ulasan telah berhasil ditambahkan!')
...
def update_ulasan_peminjam(request, ulasanid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Ulasan telah berhasil diubah!')
            return redirect('ulasanbuku:list')
    else:
        form = UlasanBukuForm(instance=ulasan)
    return render(request, 'ulasan/update_form.html', {'form': form})
